...
# (必选) 股票日线数据
stock_data_path = r'/Users/lhc/Public/Frameworks/data/stock-trading-data-2024-11-11N'
# (必选) 指数数据路径
...
if Path(stock_data_path).exists() is False:
    print(f'股票日线数据路径不存在：{stock_data_path},程序退出')
    exit()
if Path(index_data_path).exists() is False:
    print(f'指数数据路径不存在：{index_data_path},程序退出')
    exit()
